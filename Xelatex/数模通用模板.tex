...
如图\ref{fig:q1day}显示，电价有时会存在一些短期扰动，单纯的线性规划模型可能会因为一个电价的微小降价而频繁进行碎片化购电，这部分电量通常会进入储能电池。然而，对于小区微网的储能设施，剧烈改变充放电功率，会严重损耗电池寿命，带来较高的折旧成本。因此，我们建立了一个双层线性回归模型，使得在尽可能最小化购电费用的情况下，尽可能减少电池充放电的功率波动与短动作：
...
用循环总变差$\mathcal V$、启动次数$N_s$和短段缺额$B$衡量运行平稳性定义如下：
\begin{equation}
...
为保持线性结构，本文构造了一些辅助变量辅助变量$\xi_t$替代绝对值：
\begin{equation}
...
\begin{equation}
 ...
 \label{eq:q1-short}
\end{equation}
其中，$b_{t,k}^m$为模式$m$在区间$t$启动后、接下来$k$段内是否出现关闭的指示量，出现时取1，否则取0；$k=1,\ldots,K-1$为检查窗口长度，$j=1,\ldots,k$为窗口内相对启动区间的偏移。$z_{t+j}^m$指电池是否开启充电或放电模式的二元变量。$\sum_{m,t,k}$对两种模式、全天各启动位置及所有检查窗口求和，得到短段缺额$B$。启动后前$k$段内发生关闭时$b=1$，否则$b=0$。本轮$U=K=3$。
...
\begin{equation}
 ...
 \label{eq:q1-second}
\end{equation}
其中，$F_2(x)$为方案$x$的归一化运行不平稳程度，无量纲，值越小表示综合运行质量越好；三个分式依次对应功率总变差、启动次数和短段数额，各自的分母用于将指标转换到可比较的尺度。若只允许电费偏离$C^\star$一个极小数值容差，第二层可调整空间有限。故引入相对费用让步$\delta$，求解
\begin{equation}
...
通过Python代码，求解结果见表\ref{tab:q1-stages}、表\ref{tab:q1-purchase}、表\ref{tab:q1-storage}和图\ref{fig:q1day}。

\begin{table}[H]
 \centering\small
 \caption{双层混合整数规划的实际求解结果}\label{tab:q1-stages}
 \begin{tabular}{lrrrr}
 \toprule
...
图\ref{fig:q1-power}和表\ref{tab:q1-compare}显示，通过双层线性规划，电池充放电的总变差降低67.66\%，启动次数减少8次。购电费增加313.54元，增幅约0.93\%。体现了双层线性规划对保护电池寿命的突出价值。
