...
\label{sec:q1}
\subsection{问题一模型的建立}
问题一要求我们在给定负载、电价和光伏发电情况的条件下，设计一个能最小化外网购电费用的线性规划模型。其中，储能电池能够将低价时段及光伏富余时段的电量转移至用电高峰，但充放电存在10\%的损耗，容量和功率限制又使各时段决策相互关联。因此，需在全天供需平衡的基础上同时确定购电量与储能轨迹。
...
其中，$t=1,\ldots,T$为十分钟区间编号，$g_t$为区间$t$向外网申报的购电量，$w_t$为该区间未被负载或充电利用的剩余电量，单位均为kWh；$V_t$和$L_t$分别是光伏预测功率和负载预测功率，$P_t^c$与$P_t^d$分别为交流侧充电功率与放电功率，单位均为kW，且电池充放电功率应小于5000$\,kWh$。

$E_t$与$E_{t+1}$分别为区间$t$开始和结束时电池内部的储电量，单位为kWh，$1200\,kWh\leq E_t\leq10800\,kWh\quad$，$ E_1=E_{T+1}=6000\,kWh$；$\eta_c$与$\eta_d$分别为充电效率和放电效率，$\eta_{\rm rt}=\eta_c\eta_d$为往返效率。
...
\begin{equation}
 ...
 \label{eq:q1-first}
\end{equation}
其中，$x$表示由购电量、充放电功率、储电量、模式及辅助变量组成的完整调度方案，$\mathcal X$为满足上述全部约束的方案集合；$p_t$为区间$t$的已知购电单价，单位为元/kWh；$F_1(x)$为方案$x$的全天购电费用，$C^\star$为同一可行域内第一层的最优费用，单位均为元。
...
\end{equation}
其中，$\mathcal V$为含午夜边界的净功率总变差，单位为kW；$N_s$为全天充、放电模式启动总次数；$B$为短运行段相对于参考长度的累计缺额，单位为十分钟段。此式的$j$为循环运行段编号，$\ell_j^{\rm run}$为第$j$个连续同模式运行段的长度，$K=3$为短段参考长度；$(a)^+=\max(a,0)$表示取正部。
$\mathcal V$减少表示控制指令整体变化较小；$N_s$和$B$分别反映动作分散程度和短动作程度，三者均不是已经标定的电池寿命损失。
...
\end{equation}
其中，$\xi_t$为区间$t$与上一段净功率差绝对值的非负辅助上界，单位为kW；两条下界分别覆盖功率差为正和为负的情况，以线性不等式表达绝对值。
...
\begin{equation}
 ...
 \label{eq:q1-short}
\end{equation}
其中，$b_{t,k}^m$为模式$m$在区间$t$启动后、接下来$k$段内是否出现关闭的指示量，出现时取1，否则取0；$k=1,\ldots,K-1$为检查窗口长度，$j=1,\ldots,k$为窗口内相对启动区间的偏移。$\sum_{m,t,k}$对两种模式、全天各启动位置及所有检查窗口求和，得到短段缺额$B$。
...
\begin{equation}
 ...
 \label{eq:q1-second}
\end{equation}
其中，$F_2(x)$为方案$x$的归一化运行不平稳程度，无量纲，值越小表示综合运行质量越好；三个分式依次对应功率总变差、启动次数和短段缺额，各自的分母用于将指标转换到可比较的尺度。
...
\begin{equation}
 ...
 \label{eq:q1-budget}
\end{equation}
其中，$S^\star$为满足费用预算时第二层目标的最优值，无量纲；$\delta$为相对第一层最优费用$C^\star$允许的费用让步比例，$\epsilon_C$为费用约束的绝对数值容差，单位为元。
...
\begin{equation}
 ...
 \label{eq:q1-third}
\end{equation}
其中，$F_3(x)$为方案$x$的全天累计购电量，单位为kWh；$\epsilon_S$为保留第二层最优运行质量时允许的数值容差，无量纲，取$10^{-7}$。两条约束分别保留第一层费用预算与第二层运行质量，$\mathrm{s.t.}$表示“满足下列约束”。
...
\end{equation}
其中，$r_t^{\rm bal}$为区间$t$的供需能量平衡残差，$r_t^E$为该区间的储电递推残差，单位均为kWh；$r_C$为逐段重新核算的购电费用与求解目标$F_1(x)$之间的绝对差，单位为元。精确满足模型时三者均为零。
...
\begin{equation}
 ...
 \label{eq:q1-daily}
\end{equation}
其中，$Q_g=\sum_tg_t$为全天累计外网购电量，$Q_w$为全天累计未利用电量，单位均为kWh；右侧各项依次为全天负载电量、光伏电量的扣减、储能转换损耗和未利用电量。
...
\end{equation}
其中，等式左侧为全天购电量$Q_g$，右侧三个数值依次为全天负载电量$Q_L$、光伏预测电量$Q_V$和转换损耗$Q_{\rm loss}$，单位均为kWh；本方案$Q_w=0$，故未另列该项，$\simeq$表示舍入后的近似相等。
...
\end{equation}
其中，$C_{\rm stable}$为三层求解后平稳运行方案的实际全天购电费用，$C_{\rm econ}^{\star}$为不增加爬坡及最短持续时间限制时严格经济基准的第一层最优费用，单位均为元；$C^\star$仍为加入运行限制后的第一层最优费用。
