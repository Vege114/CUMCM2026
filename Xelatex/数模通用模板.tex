...
\end{equation}
其中，$m=c,d$分别表示充电和放电；$z_t^m$为相应模式是否开启的指示变量，取1表示开启，取0表示关闭。$P_{\max}=5000$\,kW为最大单向功率，$P_{\min}=1$\,kW为有效运行的计数判据，避免零功率状态被计为持续运行。由于存在二元变量，本文模型属于混合整数线性规划。
...
\begin{equation}
 ...
 \label{eq:q1-switch}
\end{equation}
其中，$s_t^m$表示模式$m$在区间$t$开始时是否启动，$o_t^m$表示是否关闭，发生时为1，否则为0。两者由前后两段的二元模式状态确定，不是可以任意取零的启动、关闭计数。
\begin{equation}
...
\begin{equation}
 ...
 \label{eq:q1-short}
\end{equation}
其中，$b_{t,k}^m$表示模式$m$在区间$t$启动后，接下来$k$段内是否出现关闭，出现时为1，否则为0；$k=1,\ldots,K-1$为检查窗口长度，$j$为窗口内的区间偏移。$\sum_{m,t,k}$对两种模式、全天各启动位置及检查窗口求和，得到短段缺额$B$。主方案取$U=K=3$，最短运行约束已使$B=0$，因此该项在主方案中不再提供额外改善，保留它是为了与允许短动作的经济基准统一比较。
...
\begin{equation}
 ...
 \label{eq:q1-second}
\end{equation}
其中，$F_2(x)$为方案$x$的归一化运行不平稳程度，无量纲，值越小表示运行越平稳；三个分式依次对应功率总变差、启动次数和短段缺额，分母用于统一数值尺度。三项等权是本文的运行偏好，不表示它们具有相同的寿命损耗。若只允许电费偏离$C^\star$一个极小数值容差，第二层可调整的空间有限，故引入相对费用让步$\delta$，求解
\begin{equation}
