...
\end{equation}
后一次发布覆盖尚未执行的计划，最终差额以午夜原计划为基准只结算一次；已执行槽不再改变。
...
\end{equation}
方案4-2中$q^{\mathrm{final}}=g^0$，调整费为零。吞吐、切换、功率变化惩罚和日末库存价值只参与规划，不计入实际账单；所有紧急购电按5倍实际价格结算。
