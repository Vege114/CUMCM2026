...
\textbf{调整费用。}
记$g^0_{d,t}$为午夜锁定原计划，$q_{d,t\mid\tau}$为本次更新后的总购电量。按相对原计划的最终净偏差定义
...
对固定$g^0_t$，常规购电费用关于$q_t$连续且分段线性：
...
\end{equation}
令$B=P_{\max}\Delta t=5000/6$\,kWh，规划中各方向的电量上限为
\begin{equation}\label{eq:q3-power-bound}
 0\le c_{s,t}\le B,\qquad 0\le d_{s,t}\le B.
\end{equation}
各情景的储能动作用于估计给定购电量下的短缺代价，最终下发的是共享购电量；真实充放电由当前供需和储电量决定。
...
\end{equation}
午夜购电项取$\Phi_0=\sum_t p_tq_t$并锁定$g^0_t=q_t$；日内取$\Phi_\tau=\sum_t p_t(1.5a_t^+-0.5a_t^-)$，省略已付且不影响优化的原计划常数。最终规划为
\begin{equation}\label{eq:q3-objective}
 ...
 \end{aligned}
\end{equation}
午夜不施加式\eqref{eq:q3-adjust-linear}。惩罚权重取$\lambda_H=0.005$元/kWh，$\lambda_{\mathrm{TV}}=0.001$元/kW。
...
\textbf{指定日期结果。}
表\ref{tab:q3-original}、\ref{tab:q3-final}分别列出午夜原计划和最终常规购电量；表\ref{tab:q3-storage-47}、\ref{tab:q3-storage-140}、\ref{tab:q3-storage-234}、\ref{tab:q3-storage-323}给出四小时充放电量及起止储电量。紧急购电区间见表\ref{tab:q3-emergency-0}与表\ref{tab:q3-emergency-2}。所有十分钟结果按区间起点展示，四小时结果为该半开区间内24槽电量之和。
...
\begin{figure}[H]
 \centering\includegraphics[width=\textwidth]{final_results/q3_purchase.pdf}
 \caption{四个指定日的午夜原计划与最终常规购电量。竖线为6、12、18时的预报发布时间。}
 \label{fig:q3-purchase}
\end{figure}
\begin{figure}[H]
 \centering\includegraphics[width=\textwidth]{final_results/q3_storage.pdf}
