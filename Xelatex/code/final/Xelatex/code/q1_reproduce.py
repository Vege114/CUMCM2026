...
def solve(data, delta=.001, ramp=1000., up=3, down=2, eta_rt=.9):
    """与 main 最新正文一致：只求费用、平稳性两层，不添加第三层。"""
    ...
    hi[ids["v"]] = 2 * PMAX  # 物理上界；避免第一层未入目标的辅助量无界。
    for name in ("c", "d"):
        hi[ids[name]] = PMAX
    ...
    records, solutions = [], []
    for stage, objective in enumerate((cost, smooth * 1e6), 1):
        ...
        pnet = x[ids["c"]] - x[ids["d"]]
        physical_smooth = (np.abs(pnet-np.roll(pnet,1)).sum()/(2*PMAX*T)
                           +sum(x[ids[k]].sum()/(2*T) for k in ("sc","sd"))
                           +sum(x[ids[k]].sum()/(4*T) for k in ("bc1","bc2","bd1","bd2")))
        rec = dict(stage=stage, cost=float(np.sum(cost*x)), quantity=float(np.sum(quantity*x)),
                   smooth=float(physical_smooth), gap=float(result.mip_gap),
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="兼容参数；默认仅求正式两层方案与经济基准")
    ...
    for name, settings in tasks:
        ...
        cases[name] = result
        save_case(out, name, data, result)
        print(json.dumps(result["metrics"], ensure_ascii=False), flush=True)
    final = cases["revised"]["trajectory"]
    summary = dict(
        environment=dict(python=sys.version, numpy=np.__version__, scipy=scipy.__version__),
        stages=cases["revised"]["stages"], metrics=cases["revised"]["metrics"],
        baseline_metrics=cases["baseline"]["metrics"],
        specified=[dict(start_hour=t/6, purchase_kwh=float(final["g"][t]))
                   for t in (60, 72, 84, 96, 108, 120)],
        storage_blocks=[dict(start_hour=4*j, end_hour=4*(j+1),
    ...
    print("完成；正式第二层轨迹与经济基准位于", out.resolve(), flush=True)
