...
**微网与外部电网电力调控策略**。截至2026-09-13，第二问已推进至exp006条件误差与动态规划/执行对照；第三、四问仍以exp002旧协议结果为主。先读[整体思路与进度同步](整体思路与进度同步_2026-09-13.md)和[团队统一建模口径](paper/团队统一建模口径.md)。

## 当前统一方案

共同主线为当时信息预测→按题目权限规划→真实执行→真实费用与运行核验。问题一三层要求继续作为统一目标；exp006尚未完成其接入。第二问优先复用exp004无季节预测，动态规划旧执行组保留费用候选身份；第三问先修正结算并建立统一基线，第四问分别迁移第二、三问。早期两阶段分析与SVG是历史设计，不作为当前实施状态。
