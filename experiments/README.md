...
按四问建立独立实验目录，公共模块集中在 `common/`。当前目录仅占位，算法和求解器待建模时确定。依赖统一由仓库根目录的 `pyproject.toml` 与 `uv.lock` 管理，环境安装和验证见 [仓库说明](../README.md#python-环境)。模型与强化学习策略网络使用 TensorFlow / `tf.keras`，强化学习环境接口使用 Gymnasium。
