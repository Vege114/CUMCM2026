...
**以 D 为直径的圆不一定覆盖区域。** 边长为 D 的正三角形的最小包围圆半径为 \(D/\sqrt3>D/2\)。因此 `D≤40 m` 不能直接作为可靠清除条件。本实现另算最小包围圆并检验覆盖半径；Q3 有 {{Q3_RADIUS_CLEAR}} 次、Q4 有 {{Q4_RADIUS_CLEAR}} 次成功清除使用了包围圆半径证据。也允许记录为 `opportunistic_estimate` 的试探性清除，失败后继续缩小区域，不把试探当作保证。
...
两局的移动时间分别占 **{{Q3_MOVE_SHARE}}** 和 **{{Q4_MOVE_SHARE}}**。问题 4 触发光学回退 **{{Q4_FALLBACKS}} 次**，对应频道 {{Q4_FALLBACK_CHANNELS}}；大量失败清除尝试属于光学格点搜索，其 3 秒光学定位成本及移动成本已全额计入。

问题 3 在清除最后一个目标后，还用了 **{{Q3_TAIL}} 秒**完成剩余频道的全域排查。这是未知总数导致的确认成本，已包含在平均时间分子中；不能在评估时删去。这也是未来优化的重要方向。
