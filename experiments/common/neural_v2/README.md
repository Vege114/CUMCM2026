...
训练使用现有 TensorFlow/Metal 环境并强制检查 GPU 输出、权重更新和模型保存读取。训练阶段顺带缓存完整月度预测，避免重复构造特征；`predict` 将它们汇总成按发布时刻、目标区间、变量、种子排列的正式档案。若月度预测文件丢失，`predict` 从已保存的 `.keras` 权重重新推理，不调用训练。检查点和预测数组的内容指纹也会核对，修改权重或上游预测不能误用已有调度结果。风险校准固定使用 1 月 25—31 日；正式费用评价为 2 月 1 日至 12 月 31 日，共 334 天。四个问题分别从共同基线的 1 月末储电量继续运行。
