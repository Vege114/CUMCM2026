"""Ten-minute mode changes with an explicit cross-midnight three-slot hold.
...
def mode_state(mask, previous_mode, previous_run_slots, minimum=3):
...
def plan(net_paths, price, soc, previous_mode=1, previous_run_slots=1,
         *, hold_slots=3, switching=50., wear=.002, seconds=5., gap=.002, final=False):
    ...
    if not LOW <= soc <= HIGH or previous_mode not in (-1, 1) or previous_run_slots < 1 or hold_slots != 3:
        raise ValueError('valid SOC, explicit previous mode/run and fixed hold3 required')
    m = LinearModel()
    q_upper = np.maximum(paths.max(0), 0.)+LIMIT
    q = m.variables((n,), upper=q_upper, cost=price)
    mode = m.variables((n,), upper=1., integer=True)
    flip = m.variables((n,), upper=1., cost=switching)
    ...
    if x is None:
        raise RuntimeError(f'No feasible variable-mode hold3 incumbent: {json.dumps(metadata)}')
    mask = x[mode] > .5
    ...
    metadata.update(method='variable_ten_minute_mode_hold3_physical_scenario_MIP',
    ...
    return {'purchase': np.maximum(x[q], 0.), 'allowed_charge': mask,
        'planned_flips': exact_flips, 'scenario_charge': x[c], 'scenario_discharge': x[d],
