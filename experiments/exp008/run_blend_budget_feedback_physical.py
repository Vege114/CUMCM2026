"""One fixed annual blend experiment with current-state Q initialization and DP.
...
from experiments.exp008.forecast_hgb_extra_trees_half import HGB, EXTRA
...
def prepare(forecast, execution_data):
    ...
    protocol = dict(candidate_count=1, evaluation_days=334, start_day=31, stop_exclusive=365,
        ...
        conditional_five_day_trial='budget_feedback/fixed5_blend_hold1_fixedQ_grid200_cap8',
    ...
    forecast_hashes, forecast_original_hashes = {}, {}
    for directory in (FORECAST_OUT,HGB,EXTRA):
        ...
        for path in artifacts:
            ...
            forecast_original_hashes[str(path)] = digest(path)
    ...
    save(OUT / 'complete_provenance.json', dict(source_sha256=hashes,
        ...
        forecast_artifact_sha256=forecast_hashes, forecast_original_sha256=forecast_original_hashes,
...
def run():
    ...
    from experiments.exp008.blend_budget_feedback_physical_audit import audit
    data = Data()
