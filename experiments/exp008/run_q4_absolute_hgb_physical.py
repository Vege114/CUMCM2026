"""Predeclared full334 Q4-2 HGB transfer with unchanged causal-price dispatch.
...
OUT = ROOT/"data/results/exp008/q4_absolute_hgb_physical"
CONFIG = {"scenario": "4-2", "forecast_calibration": PRIMARY, "holiday": False,
    ...
    "forecast_change_only": True, "this_is_not_a_Q2_goal_test": True}
...
def price_causality_check(forecast):
    ...
    for day in (31, 90, 273):
        ...
        altered = AbsoluteHGBForecasts(data=altered_data)
...
def run(out=OUT):
    ...
    baseline_path = ROOT/"data/results/exp008/q4_physical/full334/dispatch_4-2.npz"
    baseline = read_npz(baseline_path)
    files = [Path(__file__), *[ROOT/f"experiments/exp008/{name}.py" for name in
        ("mode_planning_physical", "closed_loop", "planner", "absolute_hgb_forecast_adapter",
         "forecast_absolute_hgb", "load_energy_memory", "forecast",
