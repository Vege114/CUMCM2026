...
HOLD1 = ROOT / 'data/results/exp008/mode_budget_hold1_physical/direct_hgb_memory_hold1_cap8_kappa0_334days/dispatch.npz'
...
def run():
    ...
    table = ['| 指标 | exp006 | 同 HGB 固定掩码 hold1 | 实时预算 DP |', '|---|---:|---:|---:|']
    ...
    readme = f'''# 同 HGB 预测的年度实时换向预算 DP

2025-02-01 至 2025-12-31 共 334 日、48,096 个十分钟时段连续运行完毕，并通过独立审计。实际费用较 exp006 下降 **{reduction:.6f}%**；较同 HGB 的固定掩码 hold1 方案费用差额为 **{delta_cost:,.4f} 元**。{status}
...
每日日初沿用本候选上一日的真实 SOC 和最后非空真实方向，重新求解 3 条历史路径的物理 MIP，再以全 28 条历史路径进行原固定掩码贪心目标的 120 次上限购电优化。日前 Q 锁定后，预算 DP 根据当前观测决定充放动作；其状态包括 SOC、误差档、上一非空方向及当日剩余 0–8 次换向。200 kWh 网格、3 误差档、每档 3 个尾部代表点和所有数值参数在运行前锁定。固定五日试验中未转化为实际改善的 Q 块修正没有加入本次运行。

该 Q 初始化算法依然假定一个固定模式掩码，而实际 DP 具有状态依赖的换向时机。此处保留并明确披露这个近似，没有宣称日前 Q 与最终反馈策略已经精确联合优化。3 情景 MIP 的路径补救也是乐观代理；求解间隙和未收敛的贪心优化都保存在逐日审计中。
...
独立审计核对 {audit['source_closure_files']} 个本地导入源码的启动归档和完整运行时闭包，核对所有原 HGB 来源文件、全部 334 日已发布预测和 28 条完整历史误差，重建每日 DP 模型及值表并逐元素复现实际流量。所有 334 个 MIP 的物理和模式约束均核对；{audit['future_actual_prefix_mutations']} 次非对称未来净负荷扰动保持过去动作、SOC、方向和剩余预算不变；固定 5 日的 Q 优化结果精确复现，并通过当前/未来实际及未发布未来预测扰动。最大物理残差为 {audit['max_physical_error_kwh']:.3g} kWh。第一批 3 日只按可行性和因果性过门，随后继续相同运行，已确认首批结果和计划文件完全保留。
