"""Optimize fixed purchases against the same causal inventory execution law.

Analytic reverse differentiation through clipped storage dynamics. Historical
paths are scenarios available at midnight, never the current day's actuals.
The objective is nonsmooth and nonconvex; L-BFGS-B is a local numerical method.
"""
...
def objective(purchase, net_paths, prices, initial_soc, *, charge_mask=None,
    ...
    cost=float(np.dot(purchase,prices))
    for t in range(n):
...
def optimize(initial_purchase, net_paths, prices, initial_soc, *, charge_mask=None,
             throughput=.005,variation=.001,terminal=.45,deadband=0.,maxiter=100,
             emergency_weight=5.,original=None):
    began=perf_counter()
    prices=np.asarray(prices)
    floor=np.zeros(len(prices)) if original is None else np.asarray(original)
    ...
    result=minimize(fun,np.maximum(initial_purchase,floor),jac=True,method='L-BFGS-B',
                    bounds=list(zip(floor,np.full(len(prices),np.inf))),
                    options={'maxiter':maxiter,'maxls':30,'ftol':1e-9,'gtol':1e-5})
    return {'purchase':result.x,'metadata':{'method':'closed_loop_lbfgsb',
