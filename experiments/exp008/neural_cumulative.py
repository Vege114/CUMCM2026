"""One fixed joint CNN with an added normalized cumulative net-energy loss.

Features, architecture and monthly CPU training match exp008 neural_joint.
Only the extra fixed cumulative-loss group changes. The normalization uses
known prefix length and the same training-only net-demand scale, not actual
future energy. This is a battery-inventory error proxy, not an exact monetary
loss or a global optimum certificate. Nothing is written to prior models.
"""
...
OUT=ROOT/'data/results/exp008/neural_cumulative'
CANDIDATE={'model_name':'shared_CNN_cumulative_net_huber','cumulative_group_weight':1.,'variant':'no_season','seed':42,'shared_channels':['load','pv'],
           ...
           'loss':'normalized_tariff * (mean(two symmetric channel Hubers) + net Huber + cumulative net Huber)',
           'cumulative_normalization':'cumsum(physical net error / training net scale) / sqrt(prefix length)',
...
@tf.keras.utils.register_keras_serializable(package='exp008_cumulative')
class CumulativeTariffHuber(tf.keras.losses.Loss):
    ...
    def call(self,truth,predicted):
        ...
        length=tf.cast(tf.range(1,len(self.prices)+1),predicted.dtype)
        prefix=tf.cumsum(net_error,axis=1)/tf.sqrt(length)[None,:]
        return tf.reduce_mean(weights[None,:]*(component+self.huber(net_error)+self.huber(prefix)),axis=1)
...
def build_model(prices,component_scales,net_scale):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(protocol()['training']['learning_rate']),
                  loss=CumulativeTariffHuber(prices,component_scales,net_scale),jit_compile=False)
    return model
...
def loss_check():
    ...
    loss=CumulativeTariffHuber(prices,scales,net_scale)
    ...
    prefix=np.cumsum(net,axis=1)/np.sqrt(np.arange(1,len(prices)+1))[None,:]
    expected=float(np.mean(prices/prices.mean()*(huber(y).mean(-1)+huber(net)+huber(prefix))))
...
def archive_and_score(features,directory,run_signature):
    ...
    for name,predicted in [('cnn_raw',baseline),('ridge28',ridge),('cumulative_cnn',values)]:
...
def run(months=None):
    ...
    write_json(OUT/'protocol.json',{'candidate':CANDIDATE,'inherited_training':protocol()['training'],
        ...
        'loss_override':'symmetric component, net and normalized cumulative net groups, each fixed weight one',
