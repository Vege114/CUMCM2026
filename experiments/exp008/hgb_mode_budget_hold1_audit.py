...
from experiments.exp008.run_hgb_mode_budget_hold1 import HOLD3, HOURLY, OUT, digest, save
...
def audit(expected_days=334):
    ...
    comparisons = [('exp006', baseline), ('direct_hgb_memory_hold1_cap8_kappa0', arrays)]
    hold3 = load(HOLD3/'dispatch.npz')
    assert len(hold3['states']) == 334
    hold3 = {key: value[:expected_days] for key, value in hold3.items()}
    for day in arrays['days']:
        old, new = load(HOLD3/f'planning_day{day}.npz'), load(OUT/f'planning_day{day}.npz')
    ...
    comparisons.insert(1, ('direct_hgb_memory_hold3_cap8_kappa0', hold3))
    ...
    findings = {'switch_budget_certificate': switch_budget, 'completed_days': expected_days, 'independent_audit_passed': True, 'metrics': annual,
        'hold3_same_HGB_history_comparison_passed': True,
        'mode_hold_changed_from3_to1_and_initial_two_slot_lock_removed': True,
