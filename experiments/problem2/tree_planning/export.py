"""Export the approved primary Q2 trajectory; never invoke a planner or solver.

The worksheet author is the repository's existing artifact-tool template author.
Numerical checks below are independent of both DP planning and execution code.
"""
...
def prepare():
    ...
    check = independent_check(d, check_projection=CASE == "primary", check_greedy=CASE != "primary")
    ...
    role = "预先确定的 primary 主组" if CASE == "primary" else f"{CASE} 执行器对照组"
    markdown = [f"# exp006 {role}指定四日结果", f"\n电量与储电量为 kWh，费用为元。采用{role}（无季节预测、种子 42、50 kWh 网格）。表 2 为真实负载下的执行量。全天费用包含计划购电费和 5 倍电价的紧急购电费。"]
    ...
    evidence = {"status": "prepared", "run_id": CASE, "role": "primary" if CASE == "primary" else "executor_comparator", "physical_and_billing": check,
...
'''
    ...
    run_id = "exp006" if CASE == "primary" else f"exp006/{CASE}"
...
def run(node, prepare_only=False, case="primary"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--case", choices=("primary", "greedy_execution", "fixed_primary_greedy"), default="primary")
