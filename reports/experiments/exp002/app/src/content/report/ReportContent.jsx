...
function Table({rows,columns}) {return <div className="experiment-table-scroll"><table className="experiment-table" data-reviewed-rows><thead><tr>{columns.map(([key,label])=><th key={key}>{label}</th>)}</tr></thead><tbody>{rows.map((row,i)=><tr key={i}>{columns.map(([key])=><td key={key} className={isNumeric(row[key])?"experiment-numeric":undefined}>{format(row[key])}</td>)}</tr>)}</tbody></table></div>;}
