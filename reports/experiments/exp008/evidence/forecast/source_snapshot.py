...
def historical_evidence(current):
    ...
    for number in range(1, 7):
        ...
        if number == 5:
            raw = subprocess.check_output(['git', 'show', EXP005_SPEC], cwd=ROOT)
            assert hashlib.sha256(raw).hexdigest() == EXP005_SHA
            (OUT / 'source_registry_exp005.json').write_bytes(raw)
            registry = json.loads(raw)
            origin, digest = 'git show ' + EXP005_SPEC, EXP005_SHA
        else:
    ...
    for target in TARGETS:
        for population in POPULATIONS:
            ...
            rows.append({'experiment': 'exp005', 'model': 'exp005', 'label': 'exp005 原登记·复用exp004预测',
                ...
                'source': 'git show ' + EXP005_SPEC, 'source_sha256': EXP005_SHA,
...
def run():
    ...
    save_json(OUT / 'provenance.json', {
        ...
        'historical_evidence':'Original registry rows kept separately from midnight comparable scores. exp005 from immutable git object with fixed SHA; no missing values inferred.',
