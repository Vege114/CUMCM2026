"""Prepare the existing result2 template writer's input after full certification.
...
def main():
    ...
    if status['status'] != 'complete' or status['completed_days'] != 334:
        raise RuntimeError('A complete certified 334-day policy is required before result2.xlsx export')
    audit = json.loads((OUT / 'strict_rerun/evidence/audit.json').read_text())
    assert audit['full_evaluation_complete'] and audit['completed_days'] == 334
    assert all(c['passed'] is True or c['passed'] == '原样披露' for c in audit['checks'])
    ...
    body = ['# 问题二指定四日结果', '电量、SOC单位为kWh；费用为元。普通购电计划在每天零点锁定。']
    ...
    for name, value in [('battery_blocks', blocks), ('emergency_periods', emergency_rows)]:
        (OUT / 'strict_rerun/evidence' / f'{name}.json').write_text(json.dumps(value, ensure_ascii=False, allow_nan=False))
    verification = {'result2.xlsx': {'days': 334, 'intervals_per_day': 144,
                    'total_cost': float(fees.sum()), 'violations': 0,
                    'strict_mutual_exclusion': True, 'original_template_unchanged': True,
