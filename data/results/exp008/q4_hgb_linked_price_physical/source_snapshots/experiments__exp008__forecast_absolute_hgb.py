"""One direct absolute load/PV HGB candidate; protocol saved before fitting.

Thirty-one historical/calendar inputs are the old residual HGB inputs minus
all four forecast-reference columns. No CNN/periodic output enters training
or inference. The fixed primary comparison includes identical Ridge28 and
nonrecursive gain-.5 load-memory postprocessing for HGB and joint CNN.
"""
...
OUT = ROOT / 'data/results/exp008/forecast_absolute_hgb'
PRIMARY = 'direct_hgb_ridge28_memory_half'
COMPARATOR = 'joint_ridge28_load_energy_memory_half'
...
def fit_month(data, month, features, protocol):
    ...
    return models, {'month': month, 'asof_day': asof, 'training_days': train.tolist(),
        ...
        'target_semantics': 'absolute load/PV kW throughout January and all formal months',
...
class AbsoluteHGBStore(ArrayStore):
    def __init__(self, stage=PRIMARY, directory=OUT):
        ...
        if not meta['complete'] or digest(path) != meta['archives'][stage]:
            raise ValueError('Direct HGB forecast archive is incomplete or changed')
        with np.load(path) as z:
            super().__init__(z['values'], z['origins'], stage)


def postprocess(data, values, origins):
    raw = ArrayStore(values, origins, 'direct_hgb_raw')
    ...
    ridge.name, memory.name = 'direct_hgb_ridge28', PRIMARY
    for audit in memory.audit:
        audit['underlying_forecast'] = 'direct_absolute_load_pv_HGB_then_same_fixed_Ridge28'
...
def causality_checks(data, models_by_month, raw, processed, features, protocol):
    ...
    for day in (31, 32, 59, 90, 151, 243, 364):
        ...
        future_ridge = processed['direct_hgb_ridge28'].values.copy()
        future_ridge[index + 1:] += 80000.
        memory0, _ = correct_day(data.actual, raw.origins, processed['direct_hgb_ridge28'].values, index)
    ...
    result = {'passed': True, 'future_actual_and_future_prediction_checks': future,
        ...
        'official_PV_not_loaded_by_model': True, 'CNN_or_periodic_feature_not_used': True,
        'all_postprocessing_labels_past': all(r['history_last_label'] is None or
            r['history_last_label'] < r['origin'] for r in processed['direct_hgb_ridge28'].audit)
...
def bridge():
    ...
    store = AbsoluteHGBStore()
    data = Data()
    directory = OUT / 'lp_bridge'
    case = 'direct_hgb_ridge28_memory_tree28_q08_buffer500'
...
def run():
    ...
    for month in range(2, 13):
        ...
        for day in audit['formal_days']:
            ...
            predictions.append({'day': day, 'origin': day * 144, 'month_model': month,
                ...
                'no_CNN_periodic_official_PV_future_price_input': True})
    ...
    comparison = {'joint_raw': JointStore().values, 'joint_ridge28': JointStore(calibrated=True).values,
                  COMPARATOR: EnergyMemoryStore(data=data).values}
